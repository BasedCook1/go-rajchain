// Copyright 2020 The go-rajchain Authors
...
import (
	...
	"github.com/rajchain/go-rajchain/p2p/msgrate"
...
// Syncer is an rajchain account and storage trie syncer based on snapshots and
...
// NewSyncer creates a new snapshot syncer to download the rajchain state over the
// snap protocol.
...
func (s *Syncer) processAccountResponse(res *accountResponse) {
	...
	// no longer possible in rajchain as self-destruction is disabled in Cancun
	...
	if len(res.hashes) > 0 {
		...
		for hash := range res.task.SubTasks {
			...
			// TODO(rjl493456442) degrade the log level before merging.
			// It should never happen in rajchain.
