...
// information related to the rajchain protocol such as blocks, transactions and logs.
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(rajchain.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as rajchain.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria rajchain.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(rajchain.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
