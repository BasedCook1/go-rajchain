...
import (
	...
	"github.com/rajchain/go-rajchain/core/forkid"
	...
	"github.com/rajchain/go-rajchain/eth/fetcher"
...
// support all the operations needed by the rajchain chain protocols.
...
// newHandler returns a handler for all rajchain chain management protocol.
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	// Execute the rajchain handshake
	var (
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("rajchain handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("rajchain peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("rajchain peer registration failed", "err", err)
		return err
	}
...
func (h *handler) unregisterPeer(id string) {
	...
	peer := h.peers.peer(id)
	if peer == nil {
		logger.Error("rajchain peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `eth` peer if it exists
	logger.Debug("Removing rajchain peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("rajchain peer removal failed", "err", err)
	}
}
...
func (h *handler) Stop() {
	...
	log.Info("rajchain protocol stopped")
}
