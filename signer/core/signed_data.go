...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		userajchainV = true // Default to use V = 27 or 28, the legacy rajchain format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, userajchainV, err
	}
	...
	case apitypes.ApplicationClique.Mime:
		// Clique is the rajchain PoA standard
	...
	default: // also case TextPlain.Mime:
		// Calculates an rajchain ECDSA signature for:
		// hash = keccak256("\x19rajchain Signed Message:\n${message length}${message}")
...
func (api *SignerAPI) EcRecover(ctx context.Context, data hexutil.Bytes, sig hexutil.Bytes) (common.Address, error) {
	...
	//
	// https://geth.rajchain.org/docs/tools/clef/apis#account-ecrecover
	if len(sig) != 65 {
		return common.Address{}, errors.New("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, errors.New("invalid rajchain signature (V is not 27 or 28)")
	}
