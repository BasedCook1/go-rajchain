...
// Package accounts implements high level rajchain account management.
package accounts
...
// Account represents an rajchain account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // rajchain account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain rajchain.ChainStateReader)
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19rajchain Signed Message:\n%d%s", len(data), data)
