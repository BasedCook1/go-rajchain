...
import (
	...
	"github.com/rajchain/go-rajchain/crypto/signify"
	"github.com/rajchain/go-rajchain/internal/build"
...
var (
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert rajchain contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "rajchain bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (rajchain Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "geth",
			Description: "rajchain CLI client.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "rajchain account management tool.",
		},
	}

	// A debian package is created for all executables listed here.
	debrajchain = debPackage{
		Name:        "rajchain",
...
func buildFlags(env build.Environment, staticLinking bool, buildTags []string) (flags []string) {
	...
	if env.Commit != "" {
		ld = append(ld, "-X", "github.com/rajchain/go-rajchain/internal/version.gitCommit="+env.Commit)
		ld = append(ld, "-X", "github.com/rajchain/go-rajchain/internal/version.gitDate="+env.Date)
	}
...
func downloadSpecTestFixtures(csdb *build.ChecksumDB, cachedir string) string {
	...
	url := fmt.Sprintf("https://github.com/rajchain/execution-spec-tests/releases/download/v%s/%s%s", executionSpecTestsVersion, base, ext)
...
func doCheckBadDeps() {
	baddeps := [][2]string{
		...
		{"github.com/rajchain/go-rajchain/core/rawdb", "github.com/rajchain/go-rajchain/ethdb/leveldb"},
		{"github.com/rajchain/go-rajchain/core/rawdb", "github.com/rajchain/go-rajchain/ethdb/pebbledb"},
	}
...
func doDockerBuildx(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the docker image (usually "rajchain/client-go")`)
	)
	flag.CommandLine.Parse(cmdline)
	...
	//  - rajchain/client-go:latest                            - Pushes to the master branch, Geth only
	//  - rajchain/client-go:stable                            - Version tag publish on GitHub, Geth only
	//  - rajchain/client-go:alltools-latest                   - Pushes to the master branch, Geth & tools
	//  - rajchain/client-go:alltools-stable                   - Version tag publish on GitHub, Geth & tools
	//  - rajchain/client-go:release-<major>.<minor>           - Version tag publish on GitHub, Geth only
	//  - rajchain/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, Geth & tools
	//  - rajchain/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, Geth only
	//  - rajchain/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, Geth & tools
	var tags []string
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "rajchain/rajchain")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for _, distro := range debDistros {
			// Prepare the debian package with the go-rajchain sources.
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "rajchain"
...
type debMetadata struct {
	...
	// go-rajchain version being built. Note that this
...
func newDebMetadata(distro, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "rajchain Builds <fjl@rajchain.org>"
	}
	return debMetadata{
		PackageName: name,
		Env:         env,
