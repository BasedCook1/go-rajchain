...
// Client exposes the methods provided by the rajchain RPC client.
type Client interface {
	rajchain.BlockNumberReader
	rajchain.ChainReader
	rajchain.ChainStateReader
	rajchain.ContractCaller
	rajchain.GasEstimator
	rajchain.GasPricer
	rajchain.GasPricer1559
	rajchain.FeeHistoryReader
	rajchain.LogFilterer
	rajchain.PendingStateReader
	rajchain.PendingContractCaller
	rajchain.TransactionReader
	rajchain.TransactionSender
	rajchain.ChainIDReader
}
...
// other code that interacts with the rajchain chain.
type Backend struct {
	node   *node.Node
...
func NewBackend(alloc types.GenesisAlloc, options ...func(nodeConf *node.Config, ethConf *ethconfig.Config)) *Backend {
	// Create the default configurations for the outer node shell and the rajchain
	...
	// Assemble the rajchain stack to run the chain with
