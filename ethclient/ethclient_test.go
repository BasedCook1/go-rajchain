...
import (
	...
	"github.com/rajchain/go-rajchain/ethclient"
	"github.com/rajchain/go-rajchain/node"
	"github.com/rajchain/go-rajchain/params"
	"github.com/rajchain/go-rajchain/rpc"
)

// Verify that Client implements the rajchain interfaces.
var (
	_ = rajchain.ChainReader(&ethclient.Client{})
	_ = rajchain.TransactionReader(&ethclient.Client{})
	_ = rajchain.ChainStateReader(&ethclient.Client{})
	_ = rajchain.ChainSyncReader(&ethclient.Client{})
	_ = rajchain.ContractCaller(&ethclient.Client{})
	_ = rajchain.GasEstimator(&ethclient.Client{})
	_ = rajchain.GasPricer(&ethclient.Client{})
	_ = rajchain.LogFilterer(&ethclient.Client{})
	_ = rajchain.PendingStateReader(&ethclient.Client{})
	// _ = rajchain.PendingStateEventer(&ethclient.Client{})
	_ = rajchain.PendingContractCaller(&ethclient.Client{})
)
...
func newTestBackend(config *node.Config) (*node.Node, []*types.Block, error) {
	...
	// Create rajchain Service
	ecfg := &ethconfig.Config{Genesis: genesis, RPCGasCap: 1000000}
	ethservice, err := eth.New(n, ecfg)
	if err != nil {
		return nil, nil, fmt.Errorf("can't create new rajchain service: %v", err)
	}
...
func testTransactionInBlock(t *testing.T, client *rpc.Client) {
	...
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != rajchain.NotFound {
		t.Fatal("error should be rajchain.NotFound")
	}
...
func ExampleRevertErrorData() {
	...
	call := rajchain.CallMsg{To: &contract, Gas: 30000}
